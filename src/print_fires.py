...
def main():
    ...
    if savannah_fires_column is not None or all_fires is True:
        savannah_fires_list = my_utils.get_column(
            file_name, country_column, country, result_column=savannah_fires_column
        )
    ...
    if forest_fires_column is not None or all_fires is True:
        forest_fires_list = my_utils.get_column(
            file_name, country_column, country, result_column=forest_fires_column
        )
    ...
    if organic_soil_fires_column is not None or all_fires is True:
        organic_soil_fires_list = my_utils.get_column(
            file_name, country_column, country, result_column=organic_soil_fires_column
        )
    ...
    if humid_tropical_forest_fires_column is not None or all_fires is True:
        ...
        for fire_list in fire_lists_dict.values():
            print(
                f"The median of the CO2 emissions from fires is {my_utils.array_median(fire_list)} kilotons"
            )
    ...
    try:
        ...
        print(
            "Error: No fire sources specified for the given country Please specify source(s) or use the --all_fires flag"
        )
        raise
    finally:
        if calculate_mean:
            for fire_source, fire_list in fire_lists_dict.items():
                if fire_list == []:
                    ...
                    print(
                        f"The mean of the CO2 emissions from {fire_source} fires is {my_utils.array_mean(fire_list)} kilotons"
                    )
        elif calculate_median:
            for fire_source, fire_list in fire_lists_dict.items():
                if fire_list == []:
                    ...
                    print(
                        f"The median of the CO2 emissions from {fire_source} fires is {my_utils.array_median(fire_list)} kilotons"
                    )
            ...
            for fire_source, fire_list in fire_lists_dict.items():
                if fire_list == []:
                    ...
                    print(
                        f"The standard deviation of the CO2 emissions from {fire_source} fires is {my_utils.array_stdev(fire_list)} kilotons"
                    )
        else:
            print(
                f"There was a total of {total_fires} kilotons of CO2 emissions from {fire_sources_string} fires in {country} from {minimum_year} to {maximum_year}"
            )
