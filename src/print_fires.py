...
parser = argparse.ArgumentParser(
    description=(
        "Prints the total amount of CO2 emissions from savannah, forest,\
         organic soil, and humid tropical fires in a given country from a\
         given year to a given year"
    ),
    prog="print_fires.py",
)
...
parser.add_argument(
    ...
    type=bool,
    help=(
        "To include all fire sources (savannah, forest, organic soil, humid"
        " tropical forest) in the total CO2 emissions from fires"
    ),
    nargs="?",
    const=True,
    required=False,
...
parser.add_argument(
    ...
    type=bool,
    help=(
        "To calculate the median of the CO2 emissions from the specified fire"
        " sources"
    ),
    nargs="?",
    default=False,
...
parser.add_argument(
    ...
    type=bool,
    help=(
        "To calculate the standard deviation of the CO2 emissions from the"
        " specified fire sources"
    ),
    nargs="?",
    default=False,
    required=False,
...
parser.add_argument(
    "--savannah_fires_column",
    type=int,
    help=(
        "The index of column holding the CO2 emissions from savannah fires in"
...
parser.add_argument(
    ...
    type=int,
    help=(
        "The index of column holding the CO2 emissions from forest fires in"
...
parser.add_argument(
    "--organic_soil_fires_column",
    type=int,
    help=(
        "The index of column holding the CO2 emissions from organic soil fires"
        " in the file (defaults to 22)"
    ),
    nargs="?",
    const=22,
    required=False,
...
parser.add_argument(
    ...
    type=int,
    help=(
        "The index of column holding the CO2 emissions from humid tropical"
        " forest fires in the file (defaults to 23)"
    ),
    nargs="?",
    const=23,
    required=False,
...
def main():
    ...
    if savannah_fires_column is not None or all_fires is True:
        savannah_fires_list = my_utils.get_column(
            file_name,
            country_column,
            country,
    ...
    if forest_fires_column is not None or all_fires is True:
        forest_fires_list = my_utils.get_column(
            file_name,
            country_column,
            country,
    ...
    if organic_soil_fires_column is not None or all_fires is True:
        organic_soil_fires_list = my_utils.get_column(
            file_name,
            country_column,
            country,
    ...
    if humid_tropical_forest_fires_column is not None or all_fires is True:
        humid_tropical_forest_fires_list = my_utils.get_column(
            file_name,
            country_column,
            country,
        ...
        for fire_list in fire_lists_dict.values():
            print(
                "The median of the CO2 emissions from fires is"
                f" {my_utils.array_median(fire_list)} kilotons"
            )
    ...
    try:
        if len(fire_sources) == 1:
            ...
            fire_sources_string = (
                ", ".join(fire_sources[:-1]) + " and " + fire_sources[-1]
            )
    except IndexError:
        print(
            "Error: No fire sources specified for the given country Please"
            " specify source(s) or use the --all_fires flag"
        )
        raise
    finally:
        if calculate_mean:
            for fire_source, fire_list in fire_lists_dict.items():
                if fire_list == []:
                    ...
                    print(
                        "The mean of the CO2 emissions from"
                        f" {fire_source} fires is"
                        f" {my_utils.array_mean(fire_list)} kilotons"
                    )
        elif calculate_median:
            for fire_source, fire_list in fire_lists_dict.items():
                if fire_list == []:
                    ...
                    print(
                        "The median of the CO2 emissions from"
                        f" {fire_source} fires is"
                        f" {my_utils.array_median(fire_list)} kilotons"
                    )
            ...
            for fire_source, fire_list in fire_lists_dict.items():
                if fire_list == []:
                    ...
                    print(
                        "The standard deviation of the CO2 emissions from"
                        f" {fire_source} fires is"
                        f" {my_utils.array_stdev(fire_list)} kilotons"
                    )
        else:
            print(
                f"There was a total of {total_fires} kilotons of CO2 emissions"
